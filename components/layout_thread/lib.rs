...
impl LayoutThread {
    ...
    fn notify_activity_to_hang_monitor(&self, _request: &Msg) {
        // TODO: go from request to annotation by way of a macro.
        // WRONG
        let hang_annotation = LayoutHangAnnotation::AddStylesheet;
        let msg = MonitoredComponentMsg::NotifyActivity(HangAnnotation::Layout(hang_annotation));
        self.background_hang_monitor_chan.send(msg);
    }
